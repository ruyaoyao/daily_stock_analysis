...
- [修复] 大盘复盘「国际情势/宏观背景」（费半 SOX/DXY/VIX/美债10Y）与台股「盘前展望」（TSM ADR/美股盘后）的日期对齐：原本一律用 yfinance `period='2d'` 取「最新」一根，导致非准点执行（手动补跑、深夜美股已开盘后、事后重跑）时会抓到错的美股交易日——例如 6/25 台股复盘应对应 6/24 隔夜美股，却抓到最新费半。改为亚股复盘（tw/cn/hk/jp/kr）把隔夜美系背景锁定到「复盘交易日之前最后一个收盘交易日」（取较长区间后挑正确那根），美股复盘维持当日最新；不新增日期参数、不做历史回填，准点排程（台北 18:00）行为不变。加 `before_date` 选择逻辑与对齐回归测试。
...
- [修复] 股票自动补全索引：远程索引更新固定指向 upstream（无台股），其缓存（最新）会覆盖本地内建索引，导致台股（如 1815 富喬、所有 TW/TWO）从搜索与名称解析中消失。改为载入远程索引时联集本地内建索引中远程缺失的条目（远程优先、仅补缺失键）——既保留 upstream 远程更新（CN/HK/US 等保持最新），又确保本地市场（台股）不被覆盖。加回归测试。
- [修复] Web 前端搜不到台股（如 5351 鈺創、所有 TW/TWO）：`/stocks.index.json` 路由把「最新」索引文件原样下发，当 upstream 远程缓存（无台股）最新时，下发的自动补全索引零台股；此前仅后端名称解析做了本地市场联集，下发文件未受保护。改为下发远程缓存时同样联集本地内建索引中远程缺失的条目（远程优先、仅补缺失键、按文件签名缓存避免逐请求重算），内建索引文件仍原样下发。加回归测试。
- [修复] 个股新闻去重：同一篇报道经不同来源出现时（如 1802 台玻「… - 经济日报」与「… - UDN」标题相同、URL 不同）会重复占用条数。`_rank_news_response` 新增标题去重——剥离来源后缀（Google News 风格「标题 - 来源」/「标题｜来源」）后按标题归并，保留排序最高的一条；空标题不互相归并。加回归测试锁定。
- [修复] 大盘复盘报告把「明日」写死：周末/连假前最后交易日时，展望会误写成「明日」（市场不开盘）。改为按各市场交易日历（`trading_calendar.next_trading_session`）解析「下一交易日」，非次日时用明确日期「下一交易日（YYYY-MM-DD 週X）」并附提示；日历不可用时回退为泛化措辞。A/H/美股/台股均适用。
...
- [修复] 台股大盘复盘跨数据源「日期不一致」风险：原本加權指數/成交额已是当日（6/10），但漲跌家數（openapi `STOCK_DAY_ALL`）与类股涨跌（openapi `MI_INDEX`）走的是会滞后一日的 openapi 镜像，导致复盘可能把当日指数与前一交易日的漲跌家數/类股拼在一起、判断失真。改为优先 RWD `MI_INDEX(type=ALL)`（含「当日」漲跌證券數/大盤統計/類股指數，与指数/籌碼同步），RWD 不可用时才回退 openapi；`get_tw_market_stats()` 现返回 `trade_date`（回退路径亦标注），便于跨源日期一致性校验。修复后大盘复盘的指数/成交额/漲跌家數/类股/三大法人/融資融券均落在同一交易日。
