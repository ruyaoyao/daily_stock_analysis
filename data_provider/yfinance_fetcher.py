...
class YfinanceFetcher(BaseFetcher):
    ...
    def _get_tw_main_indices(self, yf) -> Optional[List[Dict[str, Any]]]:
        """獲取台股主要指數行情（加權指數 TAIEX、櫃買指數 TPEx），復用 _fetch_yf_ticker_data。

        Yahoo Finance 台股指數符號：
        - 加權指數 TAIEX -> ^TWII
        - 櫃買指數 TPEx   -> ^TWOII
        ...
        try:
            for return_code, (yf_symbol, name) in tw_indices.items():
                try:
                    item = None
                    if return_code == 'TWII':
                        # 加權指數改用 TWSE OpenAPI（yfinance ^TWII 数据常滞后/缺漏/失真，
                        # 与官方收盘差距甚大导致涨跌幅错误）；失败时回退 yfinance。
                        try:
                            from data_provider.twse_openapi import get_tw_taiex_index
                            item = get_tw_taiex_index()
                            if item:
                                logger.debug("[TWSE] 獲取加權指數成功（OpenAPI）")
                        except Exception as twse_exc:
                            logger.warning("[TWSE] 獲取加權指數失敗，回退 yfinance: %s", twse_exc)
                    ...
                    if item is None:
                        ...
                        if item and return_code == 'TWII' and not item.get('amount'):
                            # 仅在回退 yfinance（不含成交额）时补 FMTQIK 总成交金额；
                            # get_tw_taiex_index 路径已自带「日期相符」的成交额，不在此覆盖，
                            # 避免把上一交易日成交额错贴到当日指数。
                            try:
                    ...
                    logger.warning(f"[Yfinance] 獲取台股指數 {name} 失敗: {e}")

            if results:
                logger.info(f"[Yfinance] 成功獲取 {len(results)} 個台股指數行情")
                return results

        except Exception as e:
            logger.error(f"[Yfinance] 獲取台股指數行情失敗: {e}")
    ...
    def _get_us_stock_quote_from_stooq(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        """
        使用 Stooq 為美股即時行情提供免金鑰兜底。

        Stooq 提供的是最新交易日行情，精度不如分時即時介面，但在 Yahoo / yfinance
        ...
        try:
            ...
            logger.warning(f"[Stooq] 獲取美股 {symbol} 即時行情失敗: {exc}")
            return None
    ...
    def _get_us_index_realtime_quote(
        ...
        try:
            logger.debug(f"[Yfinance] 獲取美股指數 {user_code} ({yf_symbol}) 即時行情")
            ...
            logger.info(f"[Yfinance] 獲取美股指數 {user_code} 即時行情成功: 價格={price}")
            return quote
        except Exception as e:
            logger.warning(f"[Yfinance] 獲取美股指數 {user_code} 即時行情失敗: {e}")
            return None
    ...
    def get_realtime_quote(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        ...
        try:
            ...
            logger.warning(f"[Yfinance] 獲取美股 {stock_code} 即時行情失敗: {e}，嘗試 Stooq 兜底")
