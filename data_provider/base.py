...
class DataFetcherManager:
    ...
    def get_market_stats(self, *, purpose: str = "unspecified") -> Dict[str, Any]:
        ...
        ``purpose`` 可含市场后缀（如 ``market_review:tw``），用于台股 TWSE 路由。
        """
        logger.info("[MarketStats] component=market_stats action=start purpose=%s", purpose)
        region = ""
        if ":" in purpose:
            region = purpose.rsplit(":", 1)[-1].strip().lower()
        if region == "tw":
            try:
                ...
                if data:
                    logger.info(
                        "[TWSE OpenAPI] 获取台股市场统计成功 purpose=%s",
                        purpose,
                    )
                    return data
            except Exception as e:
                logger.warning(
                    "[TWSE OpenAPI] 获取台股市场统计失败 purpose=%s error=%s",
                    purpose,
                    e,
                )
            return {}
