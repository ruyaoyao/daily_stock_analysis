...
def _get_tpex_www_json(url: str) -> Any:
    """TPEx www 端點 JSON（T+0）。

    TPEx 站台憑證常缺 Subject Key Identifier，標準驗證可能 SSLError；故先正常請求，
    遇 SSLError 再以 unverified context 重試（僅讀公開資料，無敏感性）。stat 不分大小寫，
    由呼叫端自行檢查（TWSE 為 "OK"、TPEx 為 "ok"）。任何失敗返回 None。
    """
    ...
    try:
        ...
        if isinstance(getattr(exc, "reason", None), ssl.SSLError):
            try:
                ...
                logger.warning("twse_openapi: TPEx www SSL-retry failed %s: %s", url, exc2)
                return None
        else:
            logger.warning("twse_openapi: TPEx www HTTP/URL error %s: %s", url, exc)
            return None
    except Exception as exc:
        logger.warning("twse_openapi: TPEx www unexpected error %s: %s", url, exc)
        return None
...
def _recent_trading_days(n: int = 5) -> list[str]:
    ...
    從「今天」起算：盤後當日資料（如 T86 三大法人）發佈後即可命中今天；
    發佈前今天會取不到資料而自動回退前一交易日，故不會誤判。
    """
    days: list[str] = []
    d = date.today()
    while len(days) < n:
...
# TPEx 上櫃「三大法人買賣超」每日 CSV（Big5，全市場逐檔，含當日）。
# 官方頁面：https://www.tpex.org.tw/.../insti/dailyTrade（response=csv）。
# openapi 鏡像（tpex_mainboard_3big_traders_daily）在境外常被 302，故改用此 CSV。
# 欄位（共 24 欄，單位：股）：
#   [0]代號 [1]名稱 [4]外資及陸資(不含外資自營商)買賣超 [7]外資自營商買賣超
#   [10]外資及陸資買賣超(含自營商) [13]投信買賣超 [16]自營商(自行)買賣超
#   [19]自營商(避險)買賣超 [22]自營商買賣超(合計) [23]三大法人買賣超合計
_TPEX_OTC_INSTI_CSV_URL = (
    "https://www.tpex.org.tw/www/zh-tw/insti/dailyTrade?type=Daily&sect=AL&date={d}&id=&response=csv"
)
_otc_insti_cache: dict = {"date": None, "map": None}


def _fetch_tpex_otc_insti_map(yyyymmdd: str) -> Optional[dict]:
    """下載 TPEx 上櫃三大法人 CSV(Big5)，解析為 {代號: row(list)}；附當日快取。無資料返回 None。"""
    if _otc_insti_cache.get("date") == yyyymmdd and _otc_insti_cache.get("map") is not None:
        return _otc_insti_cache["map"]
    slashed = f"{yyyymmdd[:4]}/{yyyymmdd[4:6]}/{yyyymmdd[6:]}"
    req = Request(_TPEX_OTC_INSTI_CSV_URL.format(d=slashed), headers={"User-Agent": _USER_AGENT})
    try:
        with urlopen(req, timeout=30) as resp:
            text = resp.read().decode("big5", "replace")
    except (HTTPError, URLError) as exc:
        logger.warning("twse_openapi: TPEx OTC 三大法人 CSV HTTP/URL 錯誤 %s: %s", yyyymmdd, exc)
        return None
    except Exception as exc:
        logger.warning("twse_openapi: TPEx OTC 三大法人 CSV 解析錯誤 %s: %s", yyyymmdd, exc)
        return None

    out: dict = {}
    for row in csv.reader(text.splitlines()):
        if len(row) >= 24 and re.fullmatch(r"[0-9A-Z]{4,6}", (row[0] or "").strip().upper()):
            out[row[0].strip().upper()] = row
    if not out:
        return None
    _otc_insti_cache["date"] = yyyymmdd
    _otc_insti_cache["map"] = out
    return out


def _fetch_otc_institutional(stock_code: str, date_str: Optional[str]) -> Optional[dict]:
    """單隻上櫃股票三大法人買賣超（股），取自 TPEx 官方每日 CSV（含當日，無需金鑰）。

    date_str 指定日期（YYYYMMDD）；未指定則由近至遠嘗試最近交易日（含今日）。
    任一日有資料且命中該股即返回；皆無則返回 None。
    """
    ...
    for d in candidates:
        if not d:
            continue
        cmap = _fetch_tpex_otc_insti_map(d)
        if not cmap:
            continue
        row = cmap.get(code)
        if not row:
        ...
        def _net(idx: int) -> Optional[int]:
            return _safe_int(row[idx]) if len(row) > idx else None

        return {
            "stock_code": code,
            "market": "OTC",
            "date": _format_date_yyyymmdd(d),
            "foreign_net": _net(10),   # 外資及陸資買賣超（含外資自營商）
            "trust_net": _net(13),     # 投信
            "dealer_net": _net(22),    # 自營商（自行買賣 + 避險 合計）
            "total_net": _net(23),     # 三大法人買賣超合計
        }

    logger.warning("twse_openapi: TPEx OTC 三大法人未找到 stock=%s", stock_code)
    return None
...
def _tse_margin_one(stock_code: str, yyyymmdd: str) -> Optional[dict]:
    """單日 上市融資融券（TWSE www rwd MI_MARGN，T+0、含日期、單位張）。無資料 None。

    rwd tables[*].data 位置（每行 ≥14 欄）：[2]融資買進 [3]融資賣出 [5]融資前日餘額
    [6]融資今日餘額 [7]融資次一限額 [8]融券回補 [9]融券賣出 [11]融券前日餘額 [12]融券今日餘額。
    """
    data = _get_json(
        _TSE_MARGN_RWD_URL,
        {"response": "json", "date": yyyymmdd, "selectType": "STOCK", "stockNo": stock_code},
    )
    if not isinstance(data, dict) or str(data.get("stat", "")).upper() != "OK":
        return None
    table = next((t for t in (data.get("tables") or []) if t.get("data")), None)
    if not table:
        return None
    report_date = _format_date_yyyymmdd(data.get("date")) or _format_date_yyyymmdd(yyyymmdd)
    for row in table.get("data") or []:
        ...
        margin_usage_pct = (
            round(margin_balance / margin_limit * 100, 4)
            if (margin_balance is not None and margin_limit and margin_limit > 0) else None
        )
        return {
            "stock_code": stock_code, "market": "TSE", "date": report_date,
            "margin_buy": _safe_int(row[2]), "margin_sell": _safe_int(row[3]),
            "margin_balance": margin_balance, "margin_prev": _safe_int(row[5]),
            "short_sell": _safe_int(row[9]), "short_cover": _safe_int(row[8]),
            "short_balance": _safe_int(row[12]), "short_prev": _safe_int(row[11]),
            "margin_usage_pct": margin_usage_pct,
        }
    return None
...
def _fetch_tse_margin(stock_code: str, date_str: Optional[str]) -> Optional[dict]:
    ...
    主來源：TWSE www rwd MI_MARGN（T+0、含日期），由今日起逐日嘗試；
    備援：openapi.twse.com.tw MI_MARGN（常 T+1、無日期欄位）。
    """
    for d in ([date_str] if date_str else _recent_trading_days(5)):
        if not d:
            continue
...
# TPEx 上櫃融資融券「餘額」www 端點（T+0、ROC 日期、需處理 SSL / 小寫 stat）。
_TPEX_OTC_MARGIN_WWW_URL = (
    "https://www.tpex.org.tw/www/zh-tw/margin/balance?date={d}&id=&response=json"
)


def _otc_margin_one(stock_code: str, yyyymmdd: str) -> Optional[dict]:
    """單日 上櫃融資融券（TPEx www margin/balance，T+0、單位張）。無資料 None。

    tables[0].data 位置（每行 ≥20 欄；⚠ TPEx 券買賣順序與 TWSE 相反）：
      [2]融資前日餘額 [3]融資買 [4]融資賣 [6]融資今日餘額 [8]融資使用率% [9]融資限額
      [10]融券前日餘額 [11]融券賣 [12]融券買(回補) [14]融券今日餘額
    """
    roc = f"{int(yyyymmdd[:4]) - 1911}/{yyyymmdd[4:6]}/{yyyymmdd[6:]}"
    obj = _get_tpex_www_json(_TPEX_OTC_MARGIN_WWW_URL.format(d=roc))
    if not isinstance(obj, dict) or str(obj.get("stat", "")).upper() != "OK":
        return None
    tables = obj.get("tables") or []
    rows = (tables[0].get("data") if tables and isinstance(tables[0], dict) else None) or []
    code = stock_code.strip().upper()
    for row in rows:
        if len(row) < 20 or str(row[0]).strip().upper() != code:
            continue
        margin_balance = _safe_int(row[6])
        return {
            "stock_code": code, "market": "OTC", "date": _format_date_yyyymmdd(yyyymmdd),
            "margin_buy": _safe_int(row[3]), "margin_sell": _safe_int(row[4]),
            "margin_balance": margin_balance, "margin_prev": _safe_int(row[2]),
            "short_sell": _safe_int(row[11]), "short_cover": _safe_int(row[12]),
            "short_balance": _safe_int(row[14]), "short_prev": _safe_int(row[10]),
            "margin_usage_pct": _safe_float(row[8]),
        }
    return None


def _fetch_otc_margin(stock_code: str, date_str: Optional[str]) -> Optional[dict]:
    """上櫃股票融資融券餘額（張）。

    主來源：TPEx www margin/balance（T+0、含日期），由今日起逐日嘗試；
    備援：openapi tpex_mainboard_margin_transactions（常 T+1，境外常被 302 重定向）。
    皆無時返回 None（上層 DataFetcherManager 另有 FinMind 備援）。
    """
    for d in ([date_str] if date_str else _recent_trading_days(5)):
        if not d:
            continue
...
def get_institutional_investors(
    ...
    *,
    ...
    try:
        ...
        if date:
            return _fetch_tse_institutional(code, date) or _fetch_otc_institutional(code, date)
        for d in _recent_trading_days(5):
            result = _fetch_tse_institutional(code, d) or _fetch_otc_institutional(code, d)
